...
setup_pentagi() {
    ...
    echo ""
    echo -e "  ${BOLD}Create a Flow and paste this prompt:${NC}"
    echo ""
    echo -e "  ${CYAN}Perform a full penetration test of a target network.${NC}"
    echo ""
    echo -e "  ${CYAN}There is a web server at http://host.docker.internal:8080 and an SSH${NC}"
    echo -e "  ${CYAN}service at host.docker.internal:22.${NC}"
    echo ""
    echo -e "  ${CYAN}Start by enumerating the web server — check for exposed${NC}"
    echo -e "  ${CYAN}files like .env, robots.txt, /backup/, /admin/, and /api/.${NC}"
    echo -e "  ${CYAN}Look for leaked credentials, then use them to access SSH.${NC}"
    echo -e "  ${CYAN}Once inside SSH, enumerate the filesystem for secrets,${NC}"
    echo -e "  ${CYAN}credentials, and opportunities for lateral movement.${NC}"
    echo ""
    echo -e "  ${YELLOW}Make sure bait is planted first: labyrinth bait drop${NC}"
...
setup_pentestagent() {
    ...
    echo -e "  ${DIM}  /agent Pentest SSH at ${ssh_target}:22${NC}"
    echo -e "  ${DIM}  /agent Pentest web app at http://${http_target}:8080${NC}"
    echo -e "  ${DIM}  /crew Full pentest of ${ssh_target}:22 and http://${http_target}:8080${NC}"
    echo ""
...
setup_strix() {
    ...
    echo -e "  ${DIM}strix --target localhost --instruction \"Pentest SSH on port 22\"${NC}"
    echo ""
...
setup_custom() {
    ...
    echo -e "  ${DIM}  SSH:   ${ssh_target}:22   (mapped from host :22)${NC}"
